...
#### 1. Construisez un tableau associatif
```php
<?php
	$recipe = [
	    'title' => 'Cassoulet',
...
<?php
	...
	// De la ligne 0 à la ligne 1 rechercher le contenu de la colonne 0
	for ($lines = 0; $lines <= 1; $lines++) {echo $recipes[$lines][0];}
?>	
```
#### 2. La boucle foreach 
```php
<?php
	...
	foreach ($recipes as $recipe) {echo $recipe[0];}
?>
```
#### 3. 
```php
